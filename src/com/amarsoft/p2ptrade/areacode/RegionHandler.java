 ...
 *  ItemName
 */
public class RegionHandler extends JSONHandler {
	...
	private JSONObject getInvestmentList(JSONObject request)throws HandlerException {
		if(request.get("CityId")==null || "".equals(request.get("CityId"))){
			throw new HandlerException("cityid.error");
		}
		String sCityId = request.get("CityId").toString();
		try{
			JBOFactory jbo = JBOFactory.getFactory();
		 
			BizObjectManager m =jbo.getManager("jbo.trade.code_library"); 
			BizObjectQuery query = m.createQuery(
					"select itemno,itemname from o " +
					" where codeno='AreaCode' and length(itemno)=6 and itemno like :cityid");
			query.setParameter("cityid", sCityId+"%");
