 ...
 * 
 */
public class ChargeApplyHaveContractHandler extends JSONHandler {
	ArrayList<BizObject> recordBizList = null;
	ArrayList<BizObjectManager> managerList = null;
	
	ArrayList<BizObject> contractBizList = null;
	ArrayList<BizObjectManager> contractManagerList = null;
	...
	private JSONObject runChargeApply(JSONObject request)
		...
		try {
			...
			contractBizList = new ArrayList<BizObject>();
			contractManagerList = new ArrayList<BizObjectManager>();
			...
			for (int i = 0; i < recordBizList.size(); i++) {
				if (diff1 < 0 && diff2 > 0) {// 01:00 ~ 23：00 ，实时接口充值
					...
					BizObjectManager contractManager = contractManagerList.get(i);
					BizObject contractBo = contractBizList.get(i);
...
//						recordBo.setAttributeValue("TRANSTIME",StringFunction.getNow());
	...
	private final boolean getAmountList(JBOFactory jbo,JBOTransaction tx
		...
		if (amount <= limitAmount || limitAmount == 0) {
			...
			// 生成并保存合同
			BizObjectManager contractManager = jbo.getManager(
					"jbo.trade.contract_s_record", tx);
			String sSerialNo = recordBo.getAttribute("SERIALNO").getString();
			String sInputTime = recordBo.getAttribute("INPUTTIME").getString();
			contractInfo.put("today", getCHDate(sInputTime.substring(0, 10)));
			contractInfo.put("transsum", GeneralTools.moneyFormat(amount));
			BizObject contractBo = generateAndSaveContract(contractManager, sSerialNo, sUserID, sInputTime);
			contractBizList.add(contractBo);
			contractManagerList.add(contractManager);
		} else {
			double dTemp = amount;
			while (dTemp > limitAmount) {
				...
				String sInputTime = recordBo.getAttribute("INPUTTIME").getString();
				contractInfo.put("today", getCHDate(sInputTime.substring(0, 10)));
				contractInfo.put("transsum", GeneralTools.moneyFormat(limitAmount));
				BizObject contractBo = generateAndSaveContract(contractManager, sSerialNo, sUserID, sInputTime);
				contractBizList.add(contractBo);
				contractManagerList.add(contractManager);
			}
			...
			if (dTemp > 0) {
				...
				BizObjectManager contractManager = jbo.getManager(
						"jbo.trade.contract_s_record", tx);
				String sSerialNo = recordBo.getAttribute("SERIALNO").getString();//交易流水
				String sInputTime = recordBo.getAttribute("INPUTTIME").getString();
				contractInfo.put("today", getCHDate(sInputTime.substring(0, 10)));
				contractInfo.put("transsum", GeneralTools.moneyFormat(dTemp));
				BizObject contractBo = generateAndSaveContract(contractManager, sSerialNo, sUserID, sInputTime);
				contractBizList.add(contractBo);
				contractManagerList.add(contractManager);
			}
	...
	private BizObject generateAndSaveContract(BizObjectManager contractManager, String sSerialNo, String sUserID, String sInputTime) throws HandlerException{
		try {
			BizObject contractBo = contractManager.newObject();
			contractBo.setAttributeValue("CONTRACTTYPE", "001");//充值代扣授权书
			contractBo.setAttributeValue("RELATIVENO", sSerialNo);
			contractBo.setAttributeValue("RELATIVETYPE", "001");//充值交易流水
			contractBo.setAttributeValue("SIGNUSERID", sUserID);
			contractBo.setAttributeValue("SIGNTIME", sInputTime);
			contractBo.setAttributeValue("STATUS", "2");//未生效
			contractManager.saveObject(contractBo);
		
			String sContractSerialNo = contractBo.getAttribute("SERIALNO").getString();//合同签署记录流水
		
			String htmlPath = ARE.getProperty("ContractPath")+"recharge.html";//模版HTML
		
			ARE.getLog().info(htmlPath);
	
			String outputPath = ARE.getProperty("ContractSavePath")+"Recharge/"+sContractSerialNo+".pdf";//输入html
	
			ARE.getLog().info("htmlPath = " + outputPath);
			String sFonts = ARE.getProperty("FontsPath");
			
			ARE.getLog().info("sFonts = " + sFonts);
			
			Html2PdfHandler.generatePdf(htmlPath, outputPath, ARE.getProperty("ContractPath"),contractInfo,sFonts);
			contractBo.setAttributeValue("CONTRACTNO", sContractSerialNo);//合同号和合同签署记录流水一样
			contractBo.setAttributeValue("SAVEFILE", "Recharge/"+sContractSerialNo+".pdf");
			contractManager.saveObject(contractBo);
			
			return contractBo;
		}catch (Exception e) {
			e.printStackTrace();
			throw new HandlerException("savecontractrecord.error");
		}
	}
